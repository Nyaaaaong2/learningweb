<?php
    ...
    // SELECT 조회, INSERT 삽입, DELETE 삭제, UPDATE 변경
     $sql = "INSERT INTO user (id, pw, name, address) VALUES ('$id', '$pw', '$name','$addr')";
    //$sql = "INSERT INTO `user` (`no`, `id`, `pw`, `name`, `address`) VALUES (NULL, '$id', '$pw', '$name', '$addr')";
    $result = mysqli_query($conn, $sql);
    if($result) {
        // 회원가입 후 로그인처리
        ...
        echo "<script>alert('회원가입 실패!, 다시 한번 확인해 주세요');location.href='join.php'</script>";
    }
?>
